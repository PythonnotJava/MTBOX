...
class MTFileTool:
    ...
    @staticmethod
    def _parse_markup(markup : Annotated[str, "markup is standard MTML-File's content."]) -> dict:
        # defined grammars
        editor_pattern = compile(
            r'<editor style="([^"]+)">\s*<code format="(\.[^"]+)">\s*([\s\S]+?)\s*</code>\s*</editor>')
        details_pattern = compile(r'<details style="([^"]+)">\s*([\s\S]+?)\s*</details>')
        line_pattern = compile(r'<line>\s*<parameter>(.*?)</parameter>\s*<function>(.*?)</function>\s*</line>')

        # parse the editor section
        editor_match = editor_pattern.search(markup)
        editor_style : Optional[str] = editor_match.group(1)
        editor_content : Optional[str] = editor_match.group(3)
        code_format : Optional[str] = editor_match.group(2)

        # parse the details section
        details_match = details_pattern.search(markup)
        details_style : Optional[str] = details_match.group(1)
        lines : Optional[list[dict]] = line_pattern.findall(details_match.group(2))
    ...
    def write(codes : str, **kwargs) -> str:
        ...
        if "details_style" in kwargs :
            _base_format += '<details style="{}">'.format(kwargs['details_style'])
        else:
            _base_format += '<details style="{}">'.format('font-size: 16px;color: darkblue;')
        # 'lines': [{'parameter': p0, 'function': f0}, {'parameter': p1, 'function': f2}, ...]
        if 'lines' in kwargs:
